...
To enable bounded OpenAI assistance, set `AI_PROVIDER=openai`, `EXTERNAL_AI_ALLOWED=true`, and a local `OPENAI_API_KEY`. Known SKU, terms, and cart paths use deterministic code. Ambiguous searches use `gpt-6-luna` and multi-category/project searches use `gpt-6-sol` to select among a few current catalog candidates; the returned user-facing product facts are rendered from those verified details, not model prose. Missing or failed model calls fall back to code. The gateway caps calls at 8 per session, 30 per minute, 2 concurrently, and 100 per process lifetime. Restarting resets process-local budgets; add a durable account-level budget before public deployment. The optional NVIDIA key is not required or called by this prototype.
...
- `POST /api/attachments` accepts one PDF, DOCX, XLSX, JPEG, or PNG up to 2 MiB with the same session/CSRF rules. PDFs and Office files still receive only a manual-entry warning; their contents are not parsed. JPEG/PNG files are fully decoded and re-encoded with metadata removed. The browser must send `X-Photo-Consent: true` after the customer explicitly opts into external analysis; the server must also have `EXTERNAL_AI_ALLOWED=true`. A consented photo may be sent to OpenAI for visual observations. Those observations are unverified until a fresh catalog detail matches, and the response never changes the cart. A photo upload cancels a pending cart proposal. At most four attachments are admitted concurrently, with two image jobs at once.
