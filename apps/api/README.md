...
`catalogIndex.ts` now provides a bounded, incremental in-memory candidate index with exact SKU and text lookup; tests cover 12,000 synthetic products. Category filtering works only for list rows that actually include a category, which is not guaranteed by the confirmed partner schema. The index is not yet connected to the live adapter or persisted, so current API searches still use the existing bounded catalog flow. Index entries never supply stock, price, certificate, or compatibility facts; each candidate needs a fresh detail read.
