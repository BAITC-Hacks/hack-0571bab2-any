...
2. For each `POST /api/chat`, send JSON `{ "message": "Добавь 2 ABC-123", "locale": "ru" }`, the CSRF token in `X-CSRF-Token`, and browser credentials. The browser supplies `Origin` automatically. `locale: "kk"` gives Kazakh replies and error messages; the current web UI sends Russian. The first add request returns `proposal` and `cartChanged: false`. A request such as `Добавь 2 ABC-123 и 1 ABC-124` returns both positions in `proposal.items`; negative, fractional, or ambiguous quantities do not silently become one.
3. Show every proposed item and quantity. Only a separate user action calls `POST /api/cart/confirm` with `{ "proposalId": "...", "idempotencyKey": "<fresh random UUID>" }` and the same headers. The server rereads and validates every item before changing any item; a failed check leaves the entire cart unchanged. Send optional `X-Response-Locale: kk` for Kazakh confirmation errors. Reuse the same key only when retrying that confirmation. A text message exactly confirming the current proposal also uses the same server-side confirmation checks.
4. `GET /api/cart` and `/cart` show that session's current cart. The API includes a minimal `/cart` fallback page until the web app owns the route; `/cart?lang=kk` renders it in Kazakh. The API's `cartUrl` stays `/cart`.

Demo SKUs: `ABC-123` (4 in stock), `ABC-000` (out of stock), and `ABC-124` (available candidate matching the four known critical fields of `ABC-000`). The searchable demo index also contains fictional cable `DEMO-CABLE-10`, socket `DEMO-SOCKET-16`, light `DEMO-LIGHT-12`, and switch `DEMO-SWITCH-1` so that multi-category and whole-home clarification paths work without partner credentials. Every fact still comes from the demo provider, and these items do not form a validated electrical design. Prices and certificate URLs are absent from the demo data. The chat says so explicitly; it never fabricates a certificate or amount. `GET /api/health` exposes only catalog mode and whether an AI gateway is configured; `ready` does not prove upstream availability.
