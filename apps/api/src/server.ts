...
export function buildApp(options: {
  ...
  app.post('/api/chat', async (request) => {
    ...
    const conversational = message.trim().replace(/[!?.,…]+$/gu, '').trim();
    const greeting = /^(?:привет|здравствуйте|алло(?:\s+ии)?|сәлем|сәлеметсіз бе|салем)$/iu.test(conversational);
    const aboutAssistant = /^(?:на какой я сайт попал|на каком я сайте|что за сайт|куда я попал|кто ты|что ты умеешь|сен кімсің|бұл қандай сайт)$/iu.test(conversational);
    if (greeting || aboutAssistant) return { ...basic, modelTier: 'rules', reply: locale === 'kk'
      ? 'Сәлем! Мен Электрокомплекттің ekt.kz сайтына арналған көмекшісімін. Тауар таңдауға, сипаттамаларын, бар-жоғын, баламасын және сатып алу шарттарын тексеруге көмектесемін. Қандай тауар немесе міндет үшін керек екенін жазыңыз. Артикулды білу міндетті емес.'
      : 'Здравствуйте! Я на связи. Я помощник Электрокомплекта для сайта ekt.kz. Помогу подобрать электротовар, проверить характеристики, наличие, аналоги и условия покупки. Артикул знать необязательно: опишите нужный товар или задачу.' };
    ...
    if (!product) return { ...basic, reply: sku
      ...
      : (locale === 'kk' ? 'Қандай тауар және қандай міндет үшін керек екенін жазыңыз. Артикулды білу міндетті емес. Сұрауға сәйкес нақты тауар әзірге анықталмады.' : 'Какой товар вам нужен и для какой задачи: например, кабель, автомат или освещение? Артикул знать необязательно. Пока запрос не удалось сопоставить с конкретным товаром каталога.') };
