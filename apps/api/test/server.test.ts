...
test('frontend quantity messages preserve every reviewed count in Russian and Kazakh', async (t) => {
  const app = buildApp({ catalog: createDemoCatalog(), apiOrigin: origin });
  t.after(() => app.close());
  const session = await openSession(app);
  for (const [locale, message, quantities] of [
    ['ru', 'Добавь 2 шт. артикул ABC-123', [2]],
    ['kk', 'Артикул ABC-123, себетке 2 дана қос', [2]],
    ['ru', 'Добавь 2 шт. артикул ABC-123; 1 шт. артикул ABC-124', [2, 1]],
    ['kk', 'Артикул ABC-123 2 дана; артикул ABC-124 1 дана; себетке қос', [2, 1]],
  ] as const) {
    const response = await chat(app, session, message, locale);
    assert.equal(response.statusCode, 200, message);
    assert.deepEqual(response.json().proposal?.items.map((item: { quantity: number }) => item.quantity), quantities, message);
    assert.equal((await cart(app, session)).json().itemCount, 0);
  }
  for (const message of ['Добавь -2 шт. артикул ABC-123', 'Добавь 1.5 шт. артикул ABC-123']) {
    const response = await chat(app, session, message);
    assert.equal(response.statusCode, 400, message);
    assert.equal(response.json().error.code, 'INVALID_QUANTITY', message);
  }
